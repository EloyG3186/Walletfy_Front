...
const SelectInput: React.FC<SelectProps> = ({ label, value, options, error, onChange, className }) => {
    ...
    return (
      <div className={className}>
        <label className="cd-block cd-text-lg cd-pt-2 cd-font-sans cd-font-medium cd-text-gray-900 dark:cd-text-gray-100">
          {label}
        </label>
        <select
          value={value}
        ...
        {error && <p className="cd-mt-2 cd-text-sm cd-text-red-600 dark:cd-text-red-400">{error}</p>}
      </div>
    );
  };
